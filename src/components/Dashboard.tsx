...
export function Dashboard({ currentMascot, onStartGame }: DashboardProps) {
  ...
  return (
    <div id="game-dashboard" className="bg-white text-slate-850 rounded-[48px] border-b-[12px] border-b-[#E5E7EB] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.35)] p-6 md:p-8 relative">
      <div className="absolute top-2.5 right-2.5">
        <span className="text-2xl animate-bounce inline-block">🎈</span>
      </div>

      <div className="text-center max-w-2xl mx-auto mb-8">
        <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-indigo-50 text-[#4F46E5] font-black text-xs tracking-widest uppercase mb-3 border-2 border-indigo-100">
          <Sparkles className="w-3.5 h-3.5" /> FOR THE MATH SUPERSTARS
        </span>
        <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight leading-none">
          Super Charge Your Math Brain! ⚡
        </h2>
        <p className="text-slate-600 font-extrabold text-sm mt-3">
          Choose which multiplication tables you want to test, pick an exciting game mode, and race against the clock!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 h-full flex flex-col justify-between gap-6 bg-slate-50 rounded-3xl p-6 border-2 border-slate-200 border-b-[6px]">
          <div>
            <div className="flex items-center gap-3">
              <span className="text-4xl animate-pulse">{currentMascot.emoji}</span>
              <div>
            ...
            <div className="mt-4 space-y-3 bg-white p-4 rounded-2xl border-2 border-slate-200 text-xs text-slate-750 font-medium">
